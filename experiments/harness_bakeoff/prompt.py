...
def build_prompt(icp: dict[str, Any], max_companies: int | None = None) -> str:
    ...
    return (
        ...
        "public band or an exact current integer on a fetched, identity-bound public page or recent dated filing. "
        "Stored estimates and approximate, qualified, projected, historical, or snippet-only counts are insufficient. "
        "For exact-integer proof only, map inclusive bounds deterministically: 0 or 1 -> 0-1; 2-10 -> 2-10; "
        ...
        "If the ICP lists buckets, submit only the mapped canonical employee_count when it is listed; never copy "
        "the requested bucket as proof. Include the fetched size source URL in fit_evidence_urls.\n"
        "- Every submitted company needs a verified canonical company_linkedin; U.S. HQ needs a proven state. "
        "linkedin_profile_evidence fields stand alone: never generic login/sign-up; missing employee_count/quote needs "
        "other current proof; listed_headquarters proves only its literal label. linkedin_structured_evidence is "
        "exact-identity structured proof; its size/HQ are not page quotes. A stored URL is not proof. Omit companies "
        "lacking proof.\n"
