...
def build_prompt(icp: dict[str, Any], max_companies: int | None = None) -> str:
    ...
    if contact_guidance and os.environ.get("LAB_ARENA_WORKER_SOCKET", "").strip():
        contact_guidance += (
            ...
            "screen, not full page verification. "
        )
        if primary.get("category") != "FUNDING":
            contact_guidance += (
                "When basic profile or current-stage results are still missing, get them before "
                "spending a primary-event call. Reuse existing current-stage evidence instead of "
                "requiring a new funding call, and drop a returned identity, HQ, employee-band, or "
                "stage conflict without requesting its primary event. "
            )
        contact_guidance += (
