...
import scrapingdog
from source_receipts import (FUNDING_TOOL, arena_public_web_capture, content_kind,
                             funding_record, source_date)
            ...
            try:
                ...
                for index, result in enumerate(receipts[rid].get("results", [])):
                    ...
                    if sources[ref]["content_kind"] == "captured_page" and text_field:
                        ...
                        sources[ref].update(text=source_text[:12000], total_characters=len(source_text))
                        if len(source_text) > 12000:
                            sources[ref]["continue_with"] = {"ref": ref, "field": text_field, "offset": 12000}
